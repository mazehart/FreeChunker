...
train_dataset = load_from_disk('./vector/jina-embeddings-v2-small-en/train')
# Load validation set data (assuming same structure as training set)
val_dataset_full = load_from_disk('./vector/jina-embeddings-v2-small-en/val')
...
model = FreeChunkerModel.from_pretrained('BAAI/bge-m3', ignore_mismatched_sizes=True)
model = model.to('cuda')
print('Model loaded')
...
save_dir = f'./saved_models/jina-embeddings-v2-small-en'
...
for epoch in range(num_epochs):
    ...
    for step, batch in enumerate(train_pbar, 1):
        ...
        # Print detailed info every 100 batches
        if step % 100 == 0:
    ...
    model_save_path = os.path.join(save_dir, f'qwen3_epoch_{epoch}')
...
# Also save separate step_losses file for compatibility
step_losses_path = os.path.join(save_dir, 'step_losses.json')
with open(step_losses_path, 'w') as f:
    json.dump(step_losses, f, indent=2)
print(f"Saved step losses to {step_losses_path}")

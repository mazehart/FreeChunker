...
from utils.eval_framework import truncate_chunks_by_topk
...
class UnifiedEncoder:
    ...
    def __init__(self, model_name: str, local_model_path: str = None):
        ...
        if model_name in model_configs:
            ...
            # Prioritize local path, use HF ID if not exists
            target_model = local_path if os.path.exists(local_path) else hf_id
            if target_model != local_path:
                print(f"Note: Local path {local_path} does not exist, trying to use Hugging Face model ID: {hf_id}")
    ...
    def _post_aggregation(self, sorted_indices: np.ndarray, similarities: np.ndarray, top_k: int, tokenizer=None) -> List[Tuple[str, float]]:
        ...
        texts = truncate_chunks_by_topk(tokenizer, texts)

...
class QASystem:
    def __init__(self, 
                 ...
                 embedding_model_path='/share/home/ecnuzwx/UnifiedRAG/cache/models--BAAI--bge-m3'):
...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=True, save_results=False):
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"📊 MarginSampling分块评估结果统计")
            print(f"{'='*60}")
        ...
        if save_results:
            ...
            lines = []
            lines.append(f"# MarginSampling问答结果 ({timestamp})")
            lines.append("")
            lines.append("## 总体准确率")
            lines.append("")
...
def main():
    ...
    margin_dataset_path = "/share/home/ecnuzwx/UnifiedRAG/LongBench-v2_chunked/Margin/Qwen2.5-1.5B-Instruct"
    repeats = 3
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    for embed_model in embed_models:
        ...
        log_stats(f"\n{'='*60}")
        log_stats(f"📊 MarginSampling评估结果统计（{repeats}次平均±标准差）")
        log_stats(f"{'='*60}")

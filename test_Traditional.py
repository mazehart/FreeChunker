...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=False, save_results=True):
        ...
        if save_results:
            ...
            lines = []
            lines.append(f"# Traditional问答结果 ({timestamp})")
            lines.append("")
            lines.append("## 总体准确率")
            lines.append("")
...
def main():
    ...
    for s in sizes:
        path = f"/share/home/ecnuzwx/UnifiedRAG/LongBench-v2_chunked/Traditional/{s}"
        global chunk_size
        chunk_size = s
        for embed_model in embed_models:
            ...
            log_stats(f"\n{'='*60}")
            log_stats(f"📊 Traditional评估结果统计（{repeats}次平均±标准差）")
            log_stats(f"{'='*60}")
            log_stats(f"分块大小: {s}")

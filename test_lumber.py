...
class QASystem:
    def __init__(self, 
        ...
        # 初始化监控器
        self.monitor = Monitor(device_id="2")
        self.monitor.setup()
...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=False, save_results=True):
        ...
        if save_results:
            ...
            lines = []
            lines.append(f"# Lumber问答结果 ({timestamp})")
            lines.append("")
            lines.append("## 总体准确率")
            lines.append("")
...
def main():
    ...
    for embed_model in embed_models:
        ...
        for _ in range(repeats):
            lumber_dataset_path = "/share/home/ecnuzwx/UnifiedRAG/LongBench-v2_chunked/Lumber/qwen3-8b"
            qa_system = QASystem(
        ...
        log_stats(f"\n{'='*60}")
        log_stats(f"📊 Lumber评估结果统计（{repeats}次平均±标准差）")
        log_stats(f"{'='*60}")

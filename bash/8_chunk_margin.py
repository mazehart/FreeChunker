...
# Initialize monitor
monitor = Monitor(device_id="0")

# Setup environment
monitor.setup()
model_name = 'Qwen2.5-1.5B-Instruct'
model_name_or_path = f'/share/home/FreeChunker/cache/models--Qwen--Qwen2-1.5B-Instruct'
# Initialization
print("🚀 Initializing...")
chunker = MarginSamplingChunking(model_name_or_path=model_name_or_path)

base_dir = './LongBench-v2'
selected_domains = ['single-doc']
domains = {}
for dn in selected_domains:
    p = os.path.join(base_dir, dn)
    if os.path.exists(p):
        domains[dn] = load_from_disk(p)

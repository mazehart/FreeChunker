"""
modified from lmchunker.modules.ppl_chunking
"""
...
def extract_by_html2text_db_dynamic(sub_text,model,tokenizer,threshold,threshold_zlist,language='zh') -> List[str]:  
    ...
    print('111',first_chunk_indices)
    # print('222', first_chunk_sentences)
    # temp_para grouped by perplexity
    return final_chunks,threshold,threshold_zlist

def extract_by_html2text_db_dynamic_batch(sub_text,model,tokenizer,threshold,threshold_zlist,language='zh',past_key_values=None) -> List[str]:   # non-overlapping
    temp_para=sub_text
...
def setup_model(model_name_or_path='Qwen/Qwen2-1.5B-Instruct', device="cuda"):
    """Setup model and tokenizer"""
    tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(model_name_or_path, trust_remote_code=True).to(device if device is not None else "cpu")
    model.eval()
    return model, tokenizer

def ppl_chunking(sub_text, model=None, tokenizer=None, threshold=0.5, language='en'):
...
class PPLChunking:
    def __init__(self, model_name_or_path='Qwen/Qwen2-1.5B-Instruct', device="cuda"):
        self.model, self.tokenizer = setup_model(model_name_or_path, device)

    def chunk(self, sub_text, threshold=0.5, language='en'):
        return ppl_chunking(sub_text, self.model, self.tokenizer, threshold, language)

if __name__ == "__main__":
    setup_model(device="mps")
...
class QASystem:
    def __init__(self, 
                 traditional_dataset_path=f'./LongBench-v2_chunked/Traditional/{chunk_size}',
                 embedding_model_path='jinaai/jina-embeddings-v2-small-en'):

        print("🚀 Initializing Traditional QA System...")

        print("📊 Loading Traditional dataset...")
        self.datasets = load_from_disk(traditional_dataset_path)
...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=False, save_results=True):
        ...
        if save_results:
            ...
            results_file = f"traditional_qa_results_{timestamp}.md"
            lines = []
            lines.append(f"# Traditional QA Results ({timestamp})")
            lines.append("")
...
def main():
    ...
    for s in sizes:
        path = f"../Data/LongBench-v2_chunked/Traditional/{s}"
        global chunk_size
        chunk_size = s
        for embed_model in embed_models:
            ...
            for _ in range(repeats):
                qa_system = QASystem(traditional_dataset_path=path, embedding_model_path=embed_model)
            ...
            log_stats(f"\n{'='*60}")
            log_stats(f"📊 Traditional Evaluation Stats ({repeats} runs mean±std)")
            log_stats(f"{'='*60}")
            log_stats(f"Chunk size: {s}")

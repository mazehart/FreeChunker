...
class VLLMClient:
    def __init__(self, system_prompt="You are an excellent reading comprehension assistant. Please provide answers in JSON format.", do_sample=False, temperature=0.7):
        ...
        print(f"🎲 Generation params: do_sample={do_sample}, temperature={temperature}")
...
class EncoderQASystem:
    def __init__(self, 
        ...
        print("🚀 Initializing Encoder QA System...")

        print(f"📊 Loading dataset: {dataset_path}")
        self.datasets = load_dataset(dataset_path)
        print(f"✅ Dataset loaded, domains: {list(self.datasets.keys())}")

        print(f"🔧 Initializing unified encoder: {encoder_model_name}")
        ...
        print("✅ Encoder QA System initialized")
    ...
    def process_single_question(self, sample, topk_values, verbose=True):
        """Process single question: encode once, retrieve once, then deterministic QA for different topk
        
        Args:
            sample: Question sample
            topk_values: List of topk values, e.g. [1, 3, 5]
...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=False, save_results=True):
        """Evaluate all samples - Deterministic single run"""
        
        print("🎯 Running deterministic evaluation mode")
        ...
        print(f"📊 Each sample will undergo: {len(topk_values)} topk QA rounds")
        ...
        for domain_idx, domain_name in enumerate(self.datasets, 1):
            ...
            print(f"\n{'='*80}")
            print(f"📋 Domain [{domain_idx}/{len(self.datasets)}]: {domain_name} ({total_samples} samples)")
            print('='*80)
            ...
            if verbose or True:  
                print(f"\n📊 Domain {domain_name} evaluation results:")
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"📊 Qwen3 Encoder Evaluation Statistics")
            print(f"{'='*60}")
        ...
        print(f"\n{'='*80}")
        print(f"✅ All samples processed: {processed_count}/{total_samples_all}")
        print(f"✅ Total QA count: {len(all_results)}")
        print('='*80)
        ...
        if save_results:
            ...
            lines = []
            lines.append(f"# {self.encoder_model_name} Encoder QA Results ({timestamp})")
            lines.append("")
...
def main():
    ...
    scenarios = [
        {
            'name': 'jina',
            'path': '../saved_models/2-epoch/jina-embeddings-v2-small-en/jina_epoch_1'
        },
        {
            'name': 'nomic-embed-text-v1.5',
            'path': '../saved_models/2-epoch/nomic-embed-text-v1.5/xlmroberta_epoch_1'
        },
        {
            'name': 'bge-m3',
            'path': '../saved_models/2-epoch/bge-m3/xlmroberta_epoch_1'
        },
    ]
    repeats = 1
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    for sc in scenarios:
        ...
        log_stats(f"\n{'='*60}")
        log_stats(f"📊 FreeChunker Evaluation Stats ({repeats} runs mean±std)")
        log_stats(f"{'='*60}")
        log_stats(f"Encoder: {sc['name']}")

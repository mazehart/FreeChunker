...
class QASystem: 
    def __init__(self, 
        ...
        print("🚀 Initializing PPL QA System...")

        print("📊 Loading PPL dataset...")
        ...
        print("✅ PPL QA System initialized")
...
{{
    ...
    def evaluate_single_run(self, topk_values=[1, 3, 5], verbose=True, save_results=False):
        """Evaluate single run - Test all samples"""
        print(f"🎲 Generation params: Deterministic mode")
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"📊 PPL Chunking Evaluation Statistics")
            print(f"{'='*60}")
...
def main():
    ...
    ppl_dataset_path = "../Data/LongBench-v2_chunked/PPL/Qwen2.5-1.5B-Instruct"
    repeats = 3
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    for embed_model in embed_models:
        ...
        log_stats(f"\n{'='*60}")
        log_stats(f"📊 PPL Chunking Evaluation Stats ({repeats} runs mean±std)")
        log_stats(f"{'='*60}")
